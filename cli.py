...
def main():
    ...
    if args.protocol == "ttl":
        if args.simulate:
            from hand_simulator import HandSimulator, SERVO_IDS as SIM_IDS
            ser = HandSimulator(servo_ids=SIM_IDS)
            print(f"\nUsing simulated hand ({args.hand})...")
            port_label = "SIMULATED"
            try:
                hand = HandTTL(ser, hand_type=args.hand)
            except Exception as exc:
            ...
            try:
                hand = HandTTL(port, hand_type=args.hand)
            except Exception as exc:
