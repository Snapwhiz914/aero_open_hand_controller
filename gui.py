...
try:
    from hand_view_mujoco import MuJoCoHandWidget
    _HAS_MUJOCO = True
except ImportError:
    _HAS_MUJOCO = False
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, hand: Hand, port: str, hand_type: str, serial_logger=None):
        ...
        _PARAMS_PATH = os.path.join(os.path.dirname(__file__), "hand_detector", "detector_params.json")
...
def main():
    ...
    if port == "__simulate__":
        ...
        ser  = HandSimulator(servo_ids=list(range(NUM_SERVOS)))
        hand = HandTTL(ser, hand_type=hand_type)
        port = "SIMULATED"
    elif protocol == "ttl":
        if not port:
